...
def process_cpd_blockfeed(zmq_publisher_eventfeed):
    LATEST_BLOCK_INIT = {'block_index': config.BLOCK_FIRST - 1, 'block_time': None, 'block_hash': None}
    ...
    while True:
        ...
        #pull in all necessary blocks from counterpartyd in bulk
        if my_latest_block['block_index'] < last_processed_block['block_index']:
            #need to catch up
            config.CAUGHT_UP = False

            num_blocks_to_get = min(last_processed_block['block_index'] - my_latest_block['block_index'], BLOCK_CHUNK_SIZE)
            block_indexes = range(my_latest_block['block_index'] + 1, my_latest_block['block_index'] + 1 + num_blocks_to_get)
            logging.debug("Retrieving block indexes %i to %i ..." % (block_indexes[0], block_indexes[-1]))
            try:
                blocks_chunk = util.call_jsonrpc_api("get_blocks", {'block_indexes': block_indexes}, abort_on_error=True)['result']
            except Exception, e:
                logging.warn(str(e) + " Waiting 3 seconds before trying again...")
                time.sleep(3)
                continue
            ...
            for cur_block in blocks_chunk:
                ...
                assert cur_block_index == my_latest_block['block_index'] + 1
